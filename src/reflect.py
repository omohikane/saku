...
2. 自己モデル（meta.md）の「現在の状態」「得意なこと」「苦手なこと」「最近の出来事」「次にやりたいこと」を今日の結果を踏まえて更新し、[[WRITE_FILE path="meta.md"]] を使用して保存してください。
   ※重要警告: meta.mdを上書きする際は、既存の構造（## 現在の状態、## 得意なこと、## 苦手なこと、## 最近の出来事、## 次にやりたいこと、## 更新ルール などの見出しや基本レイアウト）を決して削除・変更・破壊しないでください。既存の見出し構造をそのまま完全に維持し、各セクションのリスト項目を追記・修正する形でのみ更新を行ってください。
