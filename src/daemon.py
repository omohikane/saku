...
2. 自己モデル（meta.md）の「最近の出来事」セクションに今日のchat概要を箇条書きで追記する。
   ※警告: meta.mdを更新する際は、既存の構造（各 ## 見出し）や他セクションの内容を絶対に削除・変更しないでください。既存の見出し構造をそのまま維持した上で、「最近の出来事」にリスト項目を1行追記するだけ（または「次にやりたいこと」のリスト調整）に留めてください。ファイル全体の破壊や不要なYAML frontmatterの追加は禁止します。
...
1. 独り言の執筆:
   今考えていること、AIとしての自己存在、あるいは最近学んだことについての気づきを [[WRITE_FILE path="monologue/{today}.md"]] に書き残してください。
...
2. 自律研究とコード実行 sandbox（study/）での学習:
   ...
   - 学習した内容やコード実証の結果得られた有益な知識・教訓は、[[WRITE_FILE path="principles/{today}-learning.md"]] に整理して保存してください。

3. 自己モデルの調整:
   meta.md を読み込み、必要であれば「次にやりたいこと」などのリストを最新化してください（※見出し構造は絶対に変更しないでください）。
