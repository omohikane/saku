...
|---|---|---|---|
| **System Tools** | `saku/tools/` | Developer (you) | No (read-only for SAKU) |
| **SAKU's Own Tools** | `memory/tools/` | SAKU itself | Yes (takes priority over system tools) |

When SAKU calls a tool, `memory/tools/` is checked first. If the tool exists there, it takes precedence. Otherwise, `saku/tools/` is used as fallback.

---

## Standard System Tools

SAKU comes with a built-in set of tools located in `saku/tools/`:
...
| `DELETE_FILE` | Deletes a file in allowed directories (blocks system files and `saku/tools/`). |
...
| `GREP_CODE` | Searches Python source code in `saku/tools/` and `tools/` with regex. |
...
You can extend SAKU's capabilities by adding new Python files to the `saku/tools/` directory.

### Requirements

1. **Filename**: The filename in lowercase becomes the tool name in uppercase.
   - Example: `saku/tools/my_custom_tool.py` maps to `[[MY_CUSTOM_TOOL]]`.
...
To make the agent aware of your tool, describe its usage in `saku/core.py` under the `# Capabilities & Tools` section of `build_system_prompt()`.
